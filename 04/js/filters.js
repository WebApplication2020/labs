class Filter{
    ...
    showTasks(filterType){
        ...
        for(const task of tasks){
            ...
            const now = moment();
            if(task.deadline && task.deadline.inValid() && task.deadline.isAfter(now)){
    ...
    /**
     * Function to create a single task enclose in an <li> tag
    ...
    createTaskNode(task){
        ...
        label.className = "description custom-control-label";
        label.innerText = task.descritption;
        label.htmlFor = "check-t"+task.id;
    ...
    /**Function to destory the <ul></ul> list of tasks 
     * 
    */
   clearTasks(){
       const taskList = document.getElementById("taskList");
